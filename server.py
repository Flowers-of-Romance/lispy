...
def _spec_page(**kwargs: Any) -> str:
    """_SPEC_HTML_TEMPLATE の唯一のレンダリング入口。 view.BASE_CSS を注入し、
    /view と /spec /sessions の見た目を揃える (view 層が無い場合は空 CSS で fallback)。
    shared_css は必ず値として渡す — テンプレート文字列自体に事前連結すると CSS の
    生ブレースで .format() が壊れるので厳禁 (str.format() は渡した値の中身は
    再パースしない、テンプレート文字列だけを走査するので安全)。"""
    kwargs.setdefault("shared_css", view.BASE_CSS if view is not None else "")
    # /view のモーダル iframe 内で開かれたときのリンク調整 JS (view.py 側の定数)。
    # shared_css と同じく必ず値として渡す (JS のブレースを .format() に晒さない)。
    kwargs.setdefault("iframe_nav_js", view.IFRAME_NAV_JS if view is not None else "")
    return _SPEC_HTML_TEMPLATE.format(**kwargs)
...
def _render_sessions_html() -> str:
    ...
    def _badge(v: dict | None) -> str:
        # ダーク背景での視認性のため BASE_CSS の :root 変数色に寄せる
        # (インラインスタイル自体は残す — 一覧の各行ごとの色分けなので CSS クラス化はしない)。
        if v is None:
            return '<span style="color:var(--muted)">—</span>'
        if v.get("done"):
            return '<span style="color:var(--ok)">達成 ✅</span>'
        nxt = html.escape(v.get("next") or "")
        return '<span style="color:var(--warn)">未達 ⏳</span>' + (
        ...
        def do_GET(self):
            ...
            if path == "/view/state":
                ...
                try:
                    ...
                    # actor (executor/judge) の env 由来メタ情報 (モデル名) — 読み取り専用
                    body["actors"] = view.actors_state()
                    ...
                    # ダッシュボードの agent 俯瞰 (executor カード) 用の読み取り専用フィールド。
                    # busy = 評価中の eval が _LOCK を握っているか、 watchers = /view を
                    # 見ている SSE 接続数 (gate の答え手カウントと同じ値)。 新しい書き込みは
                    # 増やしていない — 既存の runtime 状態を素通しするだけ。
